exports.handler = async function(event, context) {
    ...
    const { avatar_id, greetingText } = JSON.parse(event.body);
    ...
    try {
        ...
        // Enhance response with WebSocket information for easier usage in frontend
        if (data.data && data.data.stream_urls) {
            // Add additional helpful information for the frontend
            data.socketInfo = {
                clientChatRoomUrl: data.data.stream_urls.client_chat_room_url,
                // Extract server ID from the URL
                serverId: data.data.stream_urls.server_chat_room_url.split('/').pop().replace('.chat', '')
            };
        }
